...
class MonsterListAdapter: SimpleRecyclerViewAdapter<Monster>() {
    ...
    override fun bindView(viewHolder: SimpleViewHolder, data: Monster) {
        val binding = FragmentListItemLargeBinding.bind(viewHolder.itemView)
        with(binding) {
            AssetLoader.setIcon(itemImage,data)
            itemLabel.text = data.name
            root.tag = data.id
            root.setOnClickListener(MonsterClickListener(viewHolder.context, data.id))
        }
    }
}
